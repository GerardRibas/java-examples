...
@Test
  ...
  public void testCreateProduct() throws JsonProcessingException, Exception {
    ...
    mvc.perform(MockMvcRequestBuilders.post("/products").accept(APPLICATION_JSON_UTF8)
        ...
        .andExpect(jsonPath("$.description", is(productSaved.getDescription()))).andExpect(header().string("Location", "http://localhost/products/1"));
  ...
  public void testFindProduct() throws JsonProcessingException, Exception {
	  Long id = 1L;
	  ProductCategoryDto category = new ProductCategoryDto(1L, "Pets", new Float("0.2"));
	  ProductDto product = new ProductDto(id, null, category, "Product", new BigDecimal("10.000"), "Blue", "10x10", "Product Description");
	  when(service.findProductById(eq(id))).thenReturn(product);
	  
	  mvc.perform(MockMvcRequestBuilders.get("/products/1")).andExpect(status().isOk())
...
@Test
  public void testFindProduct_ResourceNotFoundException() throws JsonProcessingException, Exception {
	  ...
	  mvc.perform(MockMvcRequestBuilders.get("/products/1")).andExpect(status().isNotFound())
      			.andExpect(content().contentType(APPLICATION_JSON_UTF8)).andExpect(status().isNotFound()).andExpect(jsonPath("$.code", is("E002")));
	  verify(service).findProductById(eq(id));
  }
  ...
  public void testUpdateProduct() throws Exception {
	  ...
	  mvc.perform(MockMvcRequestBuilders.put("/products/1").accept(APPLICATION_JSON_UTF8)
		        .contentType(APPLICATION_JSON_UTF8).content(objectMapper.writeValueAsString(product)))
...
@Test
  public void testUpdateProduct_ResourceNotFoundException() throws JsonProcessingException, Exception {
	  ...
	  mvc.perform(MockMvcRequestBuilders.put("/products/1").accept(APPLICATION_JSON_UTF8).contentType(APPLICATION_JSON_UTF8).content(objectMapper.writeValueAsString(product))).andExpect(status().isNotFound())
      			.andExpect(content().contentType(APPLICATION_JSON_UTF8)).andExpect(status().isNotFound()).andExpect(jsonPath("$.code", is("E002")));
	  verify(service).updateProduct(eq(product));
  }
  ...
  public void testDeleteProduct() throws Exception{
	  Long id = 1L;
	  mvc.perform(MockMvcRequestBuilders.delete("/products/1")).andExpect(status().isOk());
  ...
  public void testDeleteProduct_ResourceNotFoundException() throws Exception{
	  ...
	  mvc.perform(MockMvcRequestBuilders.delete("/products/1")).andExpect(status().isNotFound()).andExpect(content().contentType(APPLICATION_JSON_UTF8)).andExpect(status().isNotFound()).andExpect(jsonPath("$.code", is("E002")));
	  verify(service).deleteProduct(eq(id));
  }
